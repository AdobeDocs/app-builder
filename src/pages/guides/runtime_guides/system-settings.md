...
| minuteRate (actions)                        | Action rate limit per namespace per minute. Returns `429: TOO MANY REQUESTS` if exceeded.                                                          | per namespace (can be raised via support ticket)  | 6000/minute          | 18,000/min (higher values can be requested via support ticket) |
