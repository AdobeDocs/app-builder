...
The remaining parameters, `tenant` and `apiKey`, must be passed as default parameters as detailed in [Application State](./application_state.md#default-parameters). The real values would be set as environment variables in the `.env` file, mapped to the action default parameters in the `manifest.yml` file. Here are sample snippets taken from these files:
...
Return to [Introduction to React Spectrum](introduction_to_react_spectrum.md).
