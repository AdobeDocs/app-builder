## Setting up a CI/CD pipeline to deploy App Builder apps with GitHub Actions
...
GitHub Actions looks for the `.github/workflows` folder at root of your App Builder app. You can define any number of workflows or CI/CD pipelines in that folder. Check out the documentation for GitHub workflows [here](https://docs.github.com/en/actions/how-tos/write-workflows).

App Builder supplies a few workflow definitions out of the box. If you started your App Builder app using `aio app init`, the workflow YAML files will be present in the `.github/workflows` folder at the root of your App Builder app. 
...
Note: the script relies on the `.aio` and `.env` files to contain the correct values. Ensure you have completed previous steps to compute correct secret values.
...
main (){
    ...
    echo "SCOPES:                       $(echo "$config" | jq --arg ctx "$ctx" '.ims.contexts[$ctx].scopes' | jq -r -c)"
...
Note: The script outputs the variables without the `_STAGE` suffix, you'll need to add the suffix to the secret name while storing it in GitHub Secrets. You must match exactly the secret names to the names mentioned in the table.
...
2. You can freely customize the trigger for the workflow and other aspects. Read the documentation for GitHub workflows [here](https://docs.github.com/en/actions/how-tos/write-workflows).
