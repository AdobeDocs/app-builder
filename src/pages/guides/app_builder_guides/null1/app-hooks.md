...
Hooks can be defined to run before, after, or in place of many app plugin commands in the app.config.yaml or ext.config.yaml file of your app. 
...
- `aio app run` hooks manage local development tooling not managed by out-of-the-box flows.

- `aio app build` **build-static** and **build-actions** hooks:
  
  - Build actions to include static files with action zipfiles for example, templates as in server-side rendering
...
- `aio app test` **test** hooks support custom test runners for apps or extensions.
