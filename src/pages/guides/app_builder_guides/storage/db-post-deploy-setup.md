...
`aio-lib-db` uses the same index syntax as the MongoDB Node.js driver. Values for the fields in an index specification must not exceed 2048 bytes combined in a single document. Supported types follow [DocumentDB 8.0 compatibility](./db-mongo-compatibility.md):
